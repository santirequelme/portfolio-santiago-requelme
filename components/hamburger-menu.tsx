...
export function HamburgerMenu({ items }: HamburgerMenuProps) {
  ...
  return (
    <>
      <button
        onClick={toggleMenu}
        className="relative z-[1000] flex h-10 w-10 flex-col items-center justify-center gap-1.5"
          ...
          {isOpen && (
            <>
              ...
              <motion.nav
                ...
                animate="open"
                exit="closed"
                className="fixed top-0 right-0 z-[9999] h-full w-72 bg-background border-l border-border p-6 pt-20"
              >
                <button
                  onClick={toggleMenu}
                  className="absolute top-6 right-6 p-2 text-foreground/50 hover:text-foreground md:hidden"
                >
